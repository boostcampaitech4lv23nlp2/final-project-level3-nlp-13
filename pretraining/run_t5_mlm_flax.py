...
class ModelArguments:
    ...
    dtype: Optional[str] = field(
        ...
        metadata={
            "help": (
                "Floating-point format in which the model weights should be initialized and trained. Choose one of" " `[float32, float16, bfloat16]`."
            )
        },
    )
...
class FlaxDataCollatorForT5MLM:
    ...
    def __call__(self, examples: List[Dict[str, np.ndarray]]) -> BatchEncoding:
        ...
        if batch["labels"].shape[-1] != self.target_length:
            raise ValueError(
                f"`labels` are incorrectly preprocessed. `labels` length is {batch['labels'].shape[-1]}, but should be" f" {self.target_length}."
            )
...
def main():
    ...
    def group_texts(examples):
        ...
        result = {
            k: [t[i : i + expanded_inputs_length] for i in range(0, total_length, expanded_inputs_length)] for k, t in concatenated_examples.items()
        }
        return result
    ...
    def decay_mask_fn(params):
        ...
        layer_norm_named_params = set(
            [layer[-2:] for layer_norm_name in layer_norm_candidates for layer in flat_params.keys() if layer_norm_name in "".join(layer).lower()]
        )
    ...
    for epoch in epochs:
        ...
        for step, batch_idx in enumerate(tqdm(train_batch_idx, desc="Training...", position=1)):
            ...
            local_host_model_inputs = {
                key: np.split(model_inputs.data[key], num_of_hosts, axis=0)[current_host_idx] for key, value in model_inputs.data.items()
            }
            ...
            if cur_step % training_args.logging_steps == 0 and cur_step > 0:
                ...
                epochs.write(f"Step... ({cur_step} | Loss: {train_metric['loss'].mean()}, Learning Rate:" f" {train_metric['learning_rate'].mean()})")
