...
def main():
    ...
    def tokenize_function(examples):
        with CaptureLogger(tok_logger) as cl:
            ...
            output = tokenizer(examples[text_column_name], return_token_type_ids=False)  # 💥 수정
    ...
    for epoch in epochs:
        ...
        for step in tqdm(range(steps_per_epoch), desc="Training...", position=1, leave=False):
            ...
            if cur_step % training_args.eval_steps == 0 and cur_step > 0:
                ...
                desc = f"Step... ({cur_step} | Eval Loss: {eval_metrics['loss']} | Eval Perplexity:" f" {eval_metrics['perplexity']})"
                epochs.write(desc)
                epochs.desc = desc
